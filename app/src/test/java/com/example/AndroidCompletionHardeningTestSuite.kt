...
class AndroidCompletionHardeningTestSuite {
    ...
    fun testMultiTransportRoutingAndFallback() = runBlocking {
        ...
        val manager = TransportManager(listOf(btTransport, wifiDirectTransport))

        // Start transports
        manager.startAll()
        assertEquals(2, manager.transportStatuses.value.size)
        assertTrue(manager.transportStatuses.value.all { it.isAvailable })
